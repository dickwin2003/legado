...
data class RssSource(
     ...
     * 获取显示用的源名称（包含分组信息）
     * 
     * @return 如果有分组则返回"源名称 (分组)"，否则返回源名称
     */
    ...
    /**
     * 添加分组到RSS源
     * 支持添加多个分组，自动合并现有分组并去重
     * 
     * @param groups 要添加的分组字符串，多个分组用逗号分隔
     * @return 返回当前RSS源对象以支持链式调用
     */
     ...
     * 支持移除多个分组，保持其他分组不变
     * 
     * @param groups 要移除的分组字符串，多个分组用逗号分隔
     * @return 返回当前RSS源对象以支持链式调用
     */
